...
bool can_shoot()
{
    ...
    const int one = centerx - difference;
    const int two = centerx + difference;
    const int three = centery - difference;
    const int four = centery + difference;
    POINT cursor_position;
    GetCursorPos(&cursor_position);

    if (cursor_position.x < one || cursor_position.x > two || cursor_position.y < three || cursor_position.y > four)
        return false;
...
void Misc::onVoteStart(const void* data, int size) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022FTP\u2022 %c%s\x01 call vote (\x06%s\x01)", isLocal ? '\x01' : '\x06', isLocal ? "You" : entity->getPlayerName().c_str(), voteName(voteType));
}
