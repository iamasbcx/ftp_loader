...
#define __force__attribute__((always_inline))
...
namespace pbwrap {
	...
	static constexpr e_wire_type k_wire_type_for_field_type[MAX_FIELD_TYPE + 1] = {
	  (e_wire_type)(k_invalid_id), // invalid
	...
	namespace p_utils {
		...
		__forceinline auto make_tag(uint32_t field_number, uint32_t wire_type) -> uint32_t {
			return (uint32_t)((field_number << k_tag_type_bits) | wire_type);
		}
		...
		static auto read_field(const void* data, size_t& bytes_read)
			...
			uint32_t field = *(const uint16_t*)(data);
			...
			case WIRETYPE_VARINT:
				...
				value_bytes = std::string{ (const char*)((void*)((ptrdiff_t)data + bytes_read)), length };
				full_bytes = std::string{ (const char*)(data), bytes_read + length };
				bytes_read += length;
				break;
			case WIRETYPE_FIXED64:
				value_bytes = std::string{ (const char*)((void*)((ptrdiff_t)data + bytes_read)), 8 };
				full_bytes = std::string{ (const char*)(data), bytes_read + 8 };
				bytes_read += 8;
				break;
			case WIRETYPE_LENGTH_DELIMITED:
				size_delimited = read_varuint32((void*)((ptrdiff_t)data + bytes_read), length);
				value_bytes = std::string{
				  (const char*)((void*)((ptrdiff_t)data + bytes_read)), length + size_delimited
				};
				full_bytes = std::string{ (const char*)(data), bytes_read + length + size_delimited };
			...
			case WIRETYPE_FIXED32:
				value_bytes = std::string{ (const char*)((void*)((ptrdiff_t)data + bytes_read)), 4 };
				full_bytes = std::string{ (const char*)(data), bytes_read + 4 };
				bytes_read += 4;
				break;
			default:
				break;
		...
		auto get_field_bytes(uint32_t field, uint32_t wire_type, T value)
			...
			switch (wire_type) {
			case WIRETYPE_VARINT:
				bytes += get_bytes_varint64((uint64_t)(value));
				break;
			case WIRETYPE_FIXED32:
				bytes += std::string{ (const char*)(&value), 4 };
				break;
			case WIRETYPE_FIXED64:
				bytes += std::string{ (const char*)(&value), 8 };
				break;
			}
		...
		template <>
		auto get_field_bytes<std::string>(uint32_t field, uint32_t wire_type, std::string value)
			-> std::pair<std::string, std::string> {
			const auto tag = get_bytes_varint32(make_tag(field, wire_type));
		...
		template <>
		auto get_field_bytes<const char*>(uint32_t field, uint32_t wire_type, const char* value)
			-> std::pair<std::string, std::string> {
			const auto tag = get_bytes_varint32(make_tag(field, wire_type));
	...
	namespace types {
		struct Int32 {
			...
			static __forceinline auto get(const std::string& value) -> int32_t {
				size_t bytes_read;
				return p_utils::read_varuint32(value.data(), bytes_read);
			}
		};
		...
		struct Int64 {
			...
			static __forceinline auto get(const std::string& value) -> int64_t {
				size_t bytes_read;
				return p_utils::read_varuint64(value.data(), bytes_read);
			}
		};
		...
		struct Double {
			...
			static __forceinline auto get(const std::string& value) -> double {
				return *(const double*)(value.data());
			}
		};
		...
		struct Fixed32 {
			...
			static __forceinline auto get(const std::string& value) -> int32_t
			{
				return *(const int32_t*)(value.data());
			}
		};
		...
		struct Fixed64 {
			...
			static __forceinline auto get(const std::string& value) -> int64_t {
				return *(const int64_t*)(value.data());
			}
		};
		...
		struct String {
			...
			static __forceinline auto get(const std::string& value) -> std::string {
				size_t bytes_read;
				const auto length = p_utils::read_varuint32(value.data(), bytes_read);
	...
	namespace base {
		struct field_vector {
			...
			auto clear(const uint32_t field) const -> void {
				if (field >= max_size) throw std::runtime_error("field range error");

				auto& fld = fields->at(field);

				if (!fld.empty()) fld.clear();
			}
			...
			auto set(const uint32_t field, const Entry& entry, const size_t index = 0) const -> void {
				...
				if (index == 0 && fld.empty()) {
					...
					if (index >= fld.size()) throw std::runtime_error("field range error");
		...
		struct msg {
			...
			auto parse(const uint8_t* buf, size_t bytes) -> void {
				...
				while (pos < bytes) {
					std::tie(field, wire_type, field_value_bytes, field_full_bytes) = p_utils::read_field(buf + pos, bytes_read);

					if (bytes_read == 0) break;

					if (field >= fv_.max_size) throw std::runtime_error("field range error");
	...
	> : base::field<Field, Type> {
		...
		auto add(type&& value) const -> void {
			const auto pair = p_utils::get_field_bytes(hdr.field, hdr.wire_type, value);
		...
		auto set(std::vector<type>&& values) const -> void {
			...
			for (auto& v : values) {
				const auto pair = p_utils::get_field_bytes(hdr.field, hdr.wire_type, v);
				fv_.add(hdr.field, pair);
			}
		}
	...
	struct pbmsg : base::msg {
		...
		explicit pbmsg(void* buf, size_t bytes) : msg(MsgSize) {
			parse((const uint8_t*)(buf), bytes);
		}

		explicit pbmsg(const std::string& buf) : msg(MsgSize) {
			parse((const uint8_t*)(buf.data()), buf.size());
		}
	};
}