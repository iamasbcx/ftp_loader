...
void Misc::MollyHelper(ImDrawList* drawList)
{
    ...
    else if (strstr(interfaces->engine->getLevelName(), "cache"))
    {
        if (localPlayer->getActiveWeapon()->isMolotov()) {

            for (const auto& it : CacheMolly)
            {
    ...
    else if (strstr(interfaces->engine->getLevelName(), "inferno"))
    {
        if (localPlayer->getActiveWeapon()->isMolotov()) {
            ...
            for (const auto& it : InfernoMolly)
            {
